...
for line in lines:
  if count == 0:
    ...
    if "D/ST" not in line:
        ...
        playerPos = morePlayerInfo[1].replace("Recent News",r"").replace("Breaking Video",r"").replace("Breaking News",r"").replace("Breaking",r"").replace("Recent",r"")
    else:
        playerInfo = line.split();
        player = playerInfo[1] + " " + playerInfo[2]
        playerPos = playerInfo[2].replace("Recent News",r"").replace("Breaking Video",r"").replace("Breaking News",r"").replace("Breaking",r"").replace("Recent",r"")
  count = count + 1
